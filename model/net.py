...
class DiagramNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.flat_text = AttFlatText(cfg)
    ...
    def forward(self, que_emb, opt_emb, dq_matrix, dq_node_emb, dd_matrix, dd_node_emb, closest_sent_emb, cfg):
        ...
        dq_node_feat = dq_node_feat.masked_fill(dq_node_mask.unsqueeze(-1), 0.)

        dq_node_feat = self.find_dd(dq_node_feat, dd_node_feat.reshape(-1, cfg.max_dd_num, cfg.max_diagram_node,
                                                                       cfg.word_emb), cfg)
        ...
        que_feat = que_feat.masked_fill(que_mask.unsqueeze(-1) == 1, 0.)
        dq_node_feat = dq_node_feat.masked_fill(dq_node_mask.unsqueeze(-1) == 1, 0.)
        ...
        que_csf_feat = self.flat_text(que_csf, que_csf_mask, cfg)
        opt_csf_feat = self.flat_text(opt_csf, opt_csf_mask, cfg)
        que_feat = self.flat_text(que_feat, que_mask, cfg)
        opt_feat = self.flat_text(opt_feat, opt_mask, cfg)
    ...
    def find_dd(self, dq_node_feat, dd_node_feat, cfg):
        dq_ori_feat = dq_node_feat
        dq_node_feat_expand = dq_node_feat.repeat(1, cfg.max_dd_num, 1).reshape(-1, cfg.max_dd_num,
        ...
        dd_dq_node_feat, _ = self.dd_att(dq_ori_feat, closest_dd_node_feat,
                                         dq_ori_feat_mask.unsqueeze(1).unsqueeze(2),
                                         closest_dd_node_feat_mask.unsqueeze(1).unsqueeze(2))
        v = v.unsqueeze(-1).unsqueeze(-1)
        dq_node_feat_new = torch.where(v > 0.5, dd_dq_node_feat, dq_ori_feat)
        return dq_node_feat_new
