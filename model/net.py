...
class DiagramNetv2(nn.Module):
    def __init__(self, cfg):
        super(DiagramNetv2, self).__init__()
        self.lstm = nn.LSTM(
            input_size=cfg.init_word_emb,
            hidden_size=cfg.lstm_hid,
            num_layers=cfg.lstm_layer,
        ...
        self.flat = AttFlatDiagram(cfg)
        # self.ln = LayerNorm(cfg.mlp_out * 4)
    ...
    def forward(self, que_emb, opt_emb, dia_matrix, dia_node_emb, dd_matrix, dd_node_emb, closest_sent_emb, cfg):
        ...
        que_feat = que_feat.masked_fill(que_mask.unsqueeze(-1) == 1, 0.)
        dia_node_feat = dia_node_feat.masked_fill(dia_node_mask.unsqueeze(-1) == 1, 0.)
        dia_node_feat = dia_node_feat.repeat(1, cfg.max_opt_count, 1).reshape(batch_size, cfg.max_opt_count, -1,
                                                                              cfg.word_emb)
        dia_node_mask = dia_node_mask.repeat(1, cfg.max_opt_count).reshape(batch_size, cfg.max_opt_count, -1)

        opt_feat, dia_node_feat = self.intra2inter(opt_feat.reshape(-1, cfg.max_opt_len, cfg.word_emb),
                                                   dia_node_feat.reshape(-1, cfg.max_diagram_node, cfg.word_emb),
                                                   opt_mask.reshape(-1, cfg.max_opt_len).unsqueeze(1).unsqueeze(2),
                                                   dia_node_mask.reshape(-1, cfg.max_diagram_node).unsqueeze(
                                                       1).unsqueeze(2))
        opt_feat = opt_feat.reshape(batch_size, cfg.max_opt_count, cfg.max_opt_len, -1)
        opt_feat = opt_feat.masked_fill(opt_mask.unsqueeze(-1) == 1, 0.)
        ...
        fusion_feat = torch.cat((que_feat, que_csf_feat, opt_feat, opt_csf_feat), dim=-1)
        scores = self.classify(fusion_feat).squeeze(-1)
        return scores
