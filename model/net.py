...
class TextualNetBeta(nn.Module):
    ...
    def forward(self, que_emb, opt_emb, closest_sent_emb, cfg):
        ...
        # query_feat = torch.cat((flat_que, flat_opt), dim=-1)
        # context_feat = torch.cat((flat_que_csf, flat_opt_csf), dim=-1)
        # scores = self.classify(query_feat, context_feat)
    ...
    def find_sent_context(self, que_feat, opt_feat, csf_feat):
        att_q2c = torch.matmul(que_feat, csf_feat.permute(0, 2, 1))
        _, ix_q2c = torch.max(att_q2c, dim=-1)

        att_o2c = torch.matmul(opt_feat, csf_feat.permute(0, 2, 1))
        _, ix_o2c = torch.max(att_o2c, dim=-1)
        print('--')
