...
from model.mfb_csdia.config import Cfgs
from data.diagram_data_loader import DiagramDataset
from model.mfb_csdia.net import Net
from torch.nn import CrossEntropyLoss
from torch.nn.utils import clip_grad_norm_
from torch.optim import Adam, lr_scheduler
from utils.util import print_obj
from model.mfb_csdia.test_csdia import test_engine
...
def run_diagram_net(cfg):
    ...
    for epoch in range(cfg.max_epochs):
        ...
        if not os.path.exists(os.path.join(cfg.save_path, cfg.csdia_t, 'mfb')):
            os.mkdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mfb'))
        if ('ckpt_' + cfg.version) not in os.listdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mfb')):
            os.mkdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mfb', 'ckpt_' + cfg.version))
...
if __name__ == '__main__':
    ...
    parse.add_argument('--seed', type=int, default=60, help='Random seed.')
