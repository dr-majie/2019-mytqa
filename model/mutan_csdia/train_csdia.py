...
from model.mutan_csdia.config import Cfgs
from data.diagram_data_loader import DiagramDataset
from model.mutan_csdia.net import Net
from torch.nn import CrossEntropyLoss
from torch.nn.utils import clip_grad_norm_
from torch.optim import Adam, lr_scheduler
from utils.util import print_obj
from model.mutan_csdia.test_csdia import test_engine
...
def run_diagram_net(cfg):
    ...
    for epoch in range(cfg.max_epochs):
        ...
        if not os.path.exists(os.path.join(cfg.save_path, cfg.csdia_t, 'mutan')):
            os.mkdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mutan'))
        if ('ckpt_' + cfg.version) not in os.listdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mutan')):
            os.mkdir(os.path.join(cfg.save_path, cfg.csdia_t, 'mutan', 'ckpt_' + cfg.version))
